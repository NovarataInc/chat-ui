...
async function* streamingRequest(args, options) {
  ...
  info.headers["X-Amzn-SageMaker-Inference-Component"] = "meta-textgeneration-llama-2-7b-f-20240411-190231";
  console.log(JSON.stringify(info.headers));
  console.log(info.headers.toString())
  const response = await (options?.fetch ?? fetch)(url, info);
  if (options?.retry_on_error !== false && response.status === 503 && !options?.wait_for_model) {
  ...
  if (!response.ok) {
    ...
    console.log(`Test ${response.error} ${response.headers}`);
